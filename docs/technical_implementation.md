...
### 1.1 Infinity、ColPali 和 ColQwen-Omni 技术架构

#### 组件概述

msearch 系统中，Infinity、ColPali 和 ColQwen-Omni 形成了一个完整的多模态检索处理链：
...
- 高吞吐量、低延迟的 REST API 服务引擎，也是 AI 原生数据库
- 专为服务大模型设计，负责存储、索引和检索向量表示
...
- 所有生成的向量表示都存储在 Infinity 向量数据库中

**查询处理阶段**：
- 用户提交查询（可以是文本、图像或音频）
- 相应的模型（ColPali 或 ColQwen-Omni）将查询转换为向量表示
- Infinity 执行向量相似性搜索，找到最相关的向量
- 系统将检索结果返回给用户

#### 与向量数据库的关系

向量数据库是专门用于存储和检索高维向量数据的数据库系统。在 msearch 系统中，Infinity 扮演了向量数据库的角色，但它不仅仅是传统的向量数据库，而是一个 AI 原生数据库：

**与传统向量数据库的区别**：
- 传统向量数据库主要提供向量存储和相似性搜索功能
- Infinity 作为 AI 原生数据库，不仅提供向量存储和检索，还集成了模型推理能力
- Infinity 支持多种模型的 REST API 接口，可以直接处理原始数据并生成向量
...
**与 ColPali 的关系**：
ColPali 负责生成文档图像的向量表示，而 Infinity 则负责管理这些向量的存储和检索，使得系统能够高效地进行视觉文档检索。

**与 ColQwen-Omni 的关系**：
ColQwen-Omni 作为嵌入模型，负责生成多模态数据的向量表示，这些向量随后被存储在 Infinity 向量数据库中，二者共同完成多模态数据的处理和检索。
...
Infinity 的 API 与 OpenAI 的 API 规范对齐，可以通过 REST API 进行调用：
...
在 msearch 系统中，Infinity 主要用于：
1. 文本向量化处理
2. 图像向量化处理
3. 音频向量化处理
4. 视频帧向量化处理

系统通过 REST API 与 Infinity 服务通信，发送需要向量化的数据并接收对应的向量表示。
...
ColQwen-Omni 作为嵌入模型，负责生成多模态数据的向量表示。这些向量随后被存储在 Infinity 向量数据库中，以便进行高效的相似性搜索。在 msearch 系统中，ColQwen-Omni 和 Infinity 形成了一个完整的处理链：

1. ColQwen-Omni 将输入的多模态数据（文本、图像、音频）转换为高维向量
2. 这些向量通过 Infinity 进行存储、索引和检索
3. 当用户发起查询时，ColQwen-Omni 将查询转换为向量，Infinity 则负责找到最相似的向量表示
...
## 4. Infinity Python-native 集成详细实现

### 4.1 设计理念

基于设计文档中的要求，msearch 采用 Infinity Python-native 模式实现高性能本地推理：
...
#### 6.1.1 Infinity Python-native 内存管理陷阱
