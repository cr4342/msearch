...
| milvus_point_id | TEXT | - | - | 对应Milvus Lite向量数据库中的点ID |
...
2. **向量检索**: 
   - 使用查询向量在Milvus Lite中检索相似向量
   - 获取 `milvus_point_id` 列表

3. **数据关联**: 
   - 根据 `milvus_point_id` 在 `vectors` 表中查找对应的记录
