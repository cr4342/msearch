...
**问题分析**：
- **4K视频**：3840×2160分辨率，单帧数据量是720p的9倍
...
|---------|-----------|-----------|---------|---------|
| **图片文件** | 格式转换+尺寸标准化+元数据提取 | CLIP | visual_vectors | 4K→1080p降采样，减少显存压力 |
| **短视频(≤120s)** | 格式转换+分辨率降采样+关键帧提取 | CLIP | visual_vectors | HD/4K→720p转换，2秒间隔抽帧 |
| **长视频(>120s)** | 格式转换+分辨率降采样+场景检测切片 | CLIP+CLAP+Whisper | 多集合 | HD/4K→720p转换，按场景分段处理 |
...
2. **分辨率预处理**：4K/超高清图片降采样至1080p，减少显存占用
...
**关键参数**：
- **预处理尺寸**：4K→1080p，2K→1080p，HD保持原样
