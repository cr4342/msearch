...
**用户故事:** 作为视频剪辑人员，我希望在检索视频时能够快速找到相关内容并获得大致的时间位置，以便快速跳转到相关的视频片段。

#### 验收标准

1. WHEN 用户检索视频内容 THEN 系统 SHALL 返回包含时间信息的视频片段
2. WHEN 系统进行时间定位 THEN 定位精度 SHALL 在±2秒范围内（4秒范围可接受）
3. WHEN 检索结果显示视频 THEN 系统 SHALL 显示片段缩略图和大致时间位置
4. WHEN 用户点击视频结果 THEN 视频播放器 SHALL 跳转到相关时间段
5. IF 视频中有多个相似片段 THEN 系统 SHALL 按相似度排序显示结果
...
1. **预处理阶段**
   - 使用 FFmpeg 获取视频基础信息（总帧数、帧率、时长）
   - 使用 FFmpeg scene detection 检测场景变化点
   - 按场景切片为短片段（≤120秒），记录每个片段的：
     - 片段序号（从0开始）
     - 片段开始时间（秒）
     - 片段结束时间（秒）
