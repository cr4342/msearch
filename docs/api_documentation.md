...
**描述**: 执行文本检索，支持跨模态检索（文本→图像/视频/音频）。系统会自动将文本转换为向量表示，并在所有模态的向量空间中搜索相似内容。
...
| `score_threshold` | float | 否 | 0.6 | 相似度得分阈值，低于此值的结果将被过滤 |
| `filter` | object | 否 | null | 过滤条件，支持按文件类型、时间范围等过滤 |
| `modalities` | array | 否 | ["text", "image", "video", "audio"] | 指定搜索的模态类型 |
...
```json
{
  ...
  "filter": {
    "file_type": "video",
    "start_time_range": [0, 3600000]  // 只搜索前一小时内容
  },
  "modalities": ["video"]
}
```
...
**描述**: 通过上传图片进行相似图片/视频搜索，支持跨模态检索（图像→文本/图像/视频/音频）。系统会使用CLIP模型将图像转换为向量表示，并在所有模态的向量空间中搜索相似内容。

**请求格式**: 支持两种格式
1. `multipart/form-data`: 用于直接上传图像文件
2. `application/json`: 用于base64编码的图像数据

**请求参数**：

#### 对于 multipart/form-data 格式：
| 参数名 | 类型 | 必填 | 默认值 | 描述 |
|--------|------|------|--------|------|
| `file` | file | 是 | - | 图像文件（支持JPG、PNG、WEBP等格式） |
| `limit` | integer | 否 | 20 | 返回结果数量限制 |
| `score_threshold` | float | 否 | 0.6 | 相似度得分阈值 |
| `modalities` | array | 否 | ["image", "video"] | 指定搜索的模态类型 |

#### 对于 application/json 格式：
| 参数名 | 类型 | 必填 | 默认值 | 描述 |
|--------|------|------|--------|------|
| `image` | string | 是 | - | Base64编码的图像数据（格式：data:image/jpeg;base64,...） |
| `limit` | integer | 否 | 20 | 返回结果数量限制 |
| `score_threshold` | float | 否 | 0.6 | 相似度得分阈值 |
| `modalities` | array | 否 | ["image", "video"] | 指定搜索的模态类型 |
  ...
  "data": {
    ...
    "results": [
      ...
      },
      {
        "id": "point_2",
        "score": 0.85,
        "file_path": "/path/to/video_with_similar_content.mp4",
        "file_type": "video",
...
```json
{
  ...
  "data": {
    ...
    "failed_files": 50,
    "total_vectors": 50000,
    "search_requests_today": 120,
    "average_search_time": "0.25s",
    "system_load": {
      "cpu_percent": 25.0,
      "memory_percent": 68.0,
      "disk_percent": 45.0
...
```json
{
  ...
  "data": {
    ...
    "files_processed_before_stop": 1450
  }
}
```
...
class MSearchClient:
    ...
    def search_multimodal(self, query_text=None, image_path=None, audio_path=None, video_path=None, limit=10):
        """执行多模态搜索"""
        url = f"{self.base_url}/search/multimodal"
        files = {}
        data = {'limit': limit}
...
# 文本搜索
result = client.search_text("会议讨论")
...
# 多模态搜索
result = client.search_multimodal(query_text="会议场景", image_path="meeting.jpg")
...
class MSearchClient {
    ...
    async searchMultimodal(queryText, imageFile, audioFile, videoFile, limit = 10) {
        const formData = new FormData();
        formData.append('limit', limit);
...
client.searchMultimodal('会议场景', imageFile)
    .then(result => {
        console.log('多模态搜索结果:', result);
    });
```