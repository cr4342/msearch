#!/usr/bin/env python3
"""
简单测试向量化功能：将testdata中的图片向量化并存储到SQLite数据库
"""
...
def vectorize_and_store():
    ...
    if not test_pytorch_import():
        logger.error("PyTorch导入失败，无法进行向量化")
        return False
    ...
    logger.info(f"向量化测试完成！")
    logger.info(f"处理的图片数量: {processed_count}")
    logger.info(f"数据库中文件记录数: {files_count}")
    logger.info(f"数据库中向量记录数: {vectors_count}")
