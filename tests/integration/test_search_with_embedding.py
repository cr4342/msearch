#!/usr/bin/env python3
"""
使用EmbeddingEngine的检索测试
"""

import sys
import asyncio
sys.path.insert(0, 'src')

from src.core.config.config_manager import ConfigManager
from src.core.embedding.embedding_engine import EmbeddingEngine
from src.core.vector.vector_store import VectorStore

async def test_search_with_embedding():
    """使用EmbeddingEngine测试检索"""
    print("=" * 80)
    print("检索测试（使用EmbeddingEngine）")
    print("=" * 80)

    # 初始化
    print("\n初始化...")
    config_manager = ConfigManager()
    embedding_engine = EmbeddingEngine(config_manager.config)
    vector_store = VectorStore(config_manager.config)
    print("✓ 初始化完成\n")

    # 测试检索
    query_texts = ["老虎", "男人", "女人", "猫", "狗"]
    all_results = {}

    for query_text in query_texts:
        try:
            print(f"\n查询: '{query_text}'")
            print("-" * 80)

            # 生成查询向量
            print("生成查询向量...")
            query_vector = await embedding_engine.embed_text(query_text)
            print(f"向量维度: {len(query_vector)}")
            print(f"向量范围: [{min(query_vector):.3f}, {max(query_vector):.3f}]")

            # 执行检索
            print("执行检索...")
            results = vector_store.search_vectors(
                query_vector=query_vector,
                limit=10
            )

            print(f"返回结果数: {len(results)}")

            if len(results) == 0:
                print("✗ 没有返回结果")
                continue

            # 显示结果
            print("\n检索结果:")
            for i, result in enumerate(results[:10], 1):
                file_path = result.get('file_path', 'N/A')
                similarity = result.get('similarity', 0.0)
                distance = result.get('_distance', 0.0)
                print(f"  {i}. {file_path}")
                print(f"     相似度: {similarity:.3f}, 距离: {distance:.3f}")

            # 保存结果路径
            all_results[query_text] = [r.get('file_path') for r in results]

        except Exception as e:
            print(f"✗ 检索失败: {e}")
            import traceback
            traceback.print_exc()

    # 分析结果重叠
    print("\n" + "=" * 80)
    print("结果重叠分析")
    print("=" * 80)

    if len(all_results) >= 2:
        queries = list(all_results.keys())

        for i in range(len(queries)):
            for j in range(i+1, len(queries)):
                query1, query2 = queries[i], queries[j]
                results1 = all_results[query1]
                results2 = all_results[query2]

                overlap = set(results1) & set(results2)
                overlap_rate = len(overlap) / len(results1) if results1 else 0

                print(f"\n'{query1}' vs '{query2}':")
                print(f"  重叠: {len(overlap)}/{len(results1)} ({overlap_rate:.1%})")

                if overlap_rate > 0.8:
                    print(f"  ⚠️ 警告: 重叠率过高!")
                    print(f"  ⚠️ 可能原因:")
                    print(f"     1. 向量存储中文件数量过少（当前: {vector_store.get_vector_count_by_modality().get('image', 0)}）")
                    print(f"     2. 文件向量计算有问题（所有文件的向量相同或非常相似）")
                    print(f"     3. 查询向量计算有问题（所有查询的向量相同或非常相似）")

    # 检查查询向量是否不同
    print("\n" + "=" * 80)
    print("查询向量相似性分析")
    print("=" * 80)

    print("\n生成所有查询向量...")
    import numpy as np

    query_vectors = {}
    for query_text in query_texts:
        try:
            query_vector = await embedding_engine.embed_text(query_text)
            query_vectors[query_text] = np.array(query_vector)
        except Exception as e:
            print(f"✗ 生成'{query_text}'向量失败: {e}")

    # 计算查询向量之间的相似度
    if len(query_vectors) >= 2:
        queries = list(query_vectors.keys())

        print("\n查询向量之间的余弦相似度:")
        for i in range(len(queries)):
            for j in range(i+1, len(queries)):
                query1, query2 = queries[i], queries[j]
                vec1, vec2 = query_vectors[query1], query_vectors[query2]

                # 计算余弦相似度
                similarity = np.dot(vec1, vec2) / (np.linalg.norm(vec1) * np.linalg.norm(vec2))
                print(f"  '{query1}' vs '{query2}': {similarity:.3f}")

                if similarity > 0.95:
                    print(f"    ⚠️ 警告: 相似度过高，可能向量计算有问题!")

    print("\n" + "=" * 80)

if __name__ == '__main__':
    asyncio.run(test_search_with_embedding())