...
class DatabaseUpgradeTest:
    ...
    def test_new_fields_in_files_table(self) -> bool:
        ...
        try:
            # 插入包含新字段的测试文件
            test_file = {
                'file_path': '/test/upgraded_file.txt',
                'file_name': 'upgraded_file.txt',
                'file_type': 'text',
                'file_size': 1024,
                'file_category': 'document',  # 新字段
                'can_delete': True,  # 新字段
                'source_file_id': None,  # 新字段
                'hash': 'test_hash_123'
            }
    ...
    def test_file_relationships_table(self) -> bool:
        ...
        try:
            # 创建测试文件
            file1 = {
                'file_path': '/test/source_file.txt',
            ...
            file2 = {
                'file_path': '/test/related_file.txt',
                'file_name': 'related_file.txt',
                'file_type': 'text',
                'file_category': 'document',
                'hash': 'related_hash'
            }
            
            file1_id = self.db.insert_file(file1)
            file2_id = self.db.insert_file(file2)
    ...
    def test_deletable_files_functionality(self) -> bool:
        ...
        try:
            # 创建一个可删除的测试文件
            deletable_file = {
                'file_path': '/test/deletable_file.txt',
...
def main():
    """主函数"""
    print("=" * 60)
    print("🔧 数据库Schema升级验证测试")
    print("=" * 60)
    
    # 创建测试实例
    tester = DatabaseUpgradeTest()
