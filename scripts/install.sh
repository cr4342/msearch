...
EOF
    ...
    # 清理旧的API启动脚本（如果存在）
    if [ -f "${PROJECT_ROOT}/scripts/start_api.sh" ]; then
        rm -f "${PROJECT_ROOT}/scripts/start_api.sh"
    fi
...
main() {
    ...
    log_success "🚀 快速开始:"
    log_success ""
    log_success "1. 启动MSearch服务:"
    log_success "   ./scripts/start_all.sh"
    log_success ""
    log_success "2. 访问API文档:"
    log_success "   http://localhost:8000/docs"
    log_success ""
    log_success "3. 停止服务:"
    log_success "   ./scripts/stop_all.sh"
    log_success ""
    log_success "📝 注意事项:"
    log_success "- 首次使用请确保所有模型已正确下载"
    log_success "- 查看日志: tail -f logs/msearch.log"
    log_success "- 配置文件: config/config.yml"
    log_success ""
    log_success "==============================================="
    return 0
}
