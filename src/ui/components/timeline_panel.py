...
class TimelineWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.setStyleSheet(
            "background-color: #f5f5f5; border: 1px solid #ddd; border-radius: 5px;"
        )
    ...
    def _draw_time_scale(self, painter: QPainter, rect: QRectF):
        ...
        for t in range(0, int(self.video_duration) + 1, interval):
            ...
            label = f"{t}s"
            painter.drawText(
                int(x) - 20, int(rect.top() - 5), 40, 15, Qt.AlignCenter, label
            )
    ...
    def _draw_video_segments(self, painter: QPainter, rect: QRectF):
        ...
        for i, item in enumerate(self.timeline_items):
            # 计算片段位置
            start_x = (
                rect.left() + (item.start_time / self.video_duration) * rect.width()
            )
...
class TimelinePanel(QWidget):
    ...
    def init_ui(self):
        ...
        self.timeline_widget.segment_double_clicked.connect(
    ...
    def on_play_clicked(self):
        ...
        if self.timeline_items:
            # 播放当前选中的片段或第一个片段
            current_time = (
                self.time_slider.value() / 100 * self.timeline_widget.video_duration
            )
            for item in self.timeline_items:
