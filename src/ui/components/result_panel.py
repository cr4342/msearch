...
class ResultPanel(QWidget):
    ...
    def update_results_display(self):
        ...
        if filter_type != "全部":
            if filter_type == "图像":
                filtered_results = [r for r in self.results if r.get("modality") == "image"]
            elif filter_type == "视频":
                filtered_results = [r for r in self.results if r.get("modality") == "video"]
            elif filter_type == "音频":
                filtered_results = [r for r in self.results if r.get("modality") == "audio"]
        
        # 更新统计
        self.stats_label.setText(f"共找到 {len(filtered_results)} 个结果 (总计: {len(self.results)})")
