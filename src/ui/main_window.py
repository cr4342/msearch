...
# 先单独尝试导入PySide6，确保只有PySide6本身导入失败时才会设置PYSIDE6_AVAILABLE为False
...
if PYSIDE6_AVAILABLE:
    try:
        ...
        logging.error(f"其他依赖导入失败: {e}")
        # 如果其他依赖导入失败，定义一个简化的MainWindow类
        class MainWindow(QMainWindow):
            def __init__(self):
                super().__init__()
                self.setWindowTitle("msearch - 多模态检索系统")
                ...
                label = QLabel("系统初始化失败，请检查日志获取详细信息")
