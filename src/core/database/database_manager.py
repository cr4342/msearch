...
class DatabaseManager:
    ...
    def create_tables(self) -> None:
        ...
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_file_status ON file_metadata(processing_status)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_ref_hash ON file_references(file_hash)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_video_file_id ON video_metadata(file_id)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_video_segments_video_id ON video_segments(video_id)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_vector_file_id ON vector_timestamp_map(file_id)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_vector_modality ON vector_timestamp_map(modality)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_vector_timestamp ON vector_timestamp_map(timestamp)')
    ...
    def update_file_status(self, file_id: str, status: str) -> bool:
        ...
        Returns:
            是否成功
        """
        return self.update_file_metadata(file_id, {
            'processing_status': status,
            'processed_at': datetime.now().timestamp() if status == 'completed' else None
        })
    ...
    def insert_vector_timestamp(self, vector_id: str, file_id: str, segment_id: str, 
                                modality: str, timestamp: float, confidence: float = 1.0) -> str:
        """
        插入向量时间戳映射
        ...
        try:
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (map_id, vector_id, file_id, segment_id, modality, timestamp, confidence, now))
    ...
    def get_vectors_by_time_range(self, file_id: str, start_time: float, 
                                   end_time: float, modality: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        获取指定时间范围内的向量
    ...
    def get_indexed_files(self) -> int:
        ...
        try:
            cursor = self.connection.cursor()
            cursor.execute('SELECT COUNT(*) FROM file_metadata WHERE processing_status = "completed"')
            count = cursor.fetchone()[0]
            return count
        except Exception as e:
    ...
    def get_all_files(self) -> List[Dict[str, Any]]:
        ...
        try:
            cursor = self.connection.cursor()
            cursor.execute('SELECT id, file_path, file_name, file_type FROM file_metadata')
            files = []
            for row in cursor.fetchall():
                files.append({
                    'id': row[0],
