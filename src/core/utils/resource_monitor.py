...
class ResourceUsage:
    ...
    def is_over_threshold(
        self, cpu_threshold: float = 90, memory_threshold: float = 90
    ) -> bool:
        """检查是否超过阈值"""
        return (
            self.cpu_percent > cpu_threshold or self.memory_percent > memory_threshold
        )
...
class ResourceMonitor:
    ...
    async def _check_thresholds(self, usage: ResourceUsage):
        ...
        if self._is_paused:
            if (
                usage.cpu_percent < self.CPU_WARNING_THRESHOLD - 5
                and usage.memory_percent < self.MEMORY_WARNING_THRESHOLD - 5
            ):
                logger.info(
        ...
        # 警告（80%阈值）
        if (
            usage.cpu_percent > self.CPU_WARNING_THRESHOLD
            or usage.memory_percent > self.MEMORY_WARNING_THRESHOLD
        ):
    ...
    def get_resource_trend(self) -> Dict[str, Any]:
        ...
        # 计算CPU趋势
        cpu_trend = (
            self._history[-1].cpu_percent - self._history[0].cpu_percent
        ) / len(self._history)

        # 计算内存趋势
        memory_trend = (
            self._history[-1].memory_percent - self._history[0].memory_percent
        ) / len(self._history)
        ...
        return {
            ...
            "trend": (
                "increasing" if cpu_trend > 0 or memory_trend > 0 else "decreasing"
            ),
        }
