...
class EmbeddingEngine:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # 批处理优化器
        batch_size = self.models_config.get('available_models', {}).get(
            self._get_default_model_id(), {}
        ).get('batch_size', 8)
        ...
        logger.info(f"向量化引擎初始化完成，配置驱动模式，模型数量: {len(self._model_configs)}")
    ...
    def _init_model_configs(self):
        ...
        if active_models:
            ...
            for model_id in active_models:
                ...
                if 'audio' in model_id.lower() or 'clap' in model_config.get('model_name', '').lower():
    ...
    async def _get_model_client(self, model_type: str = None):
        ...
        if model_type not in self._model_manager.get_loaded_models():
            raise ValueError(f"未找到模型: {model_type}。已加载的模型: {self._model_manager.get_loaded_models()}")
    ...
    def _validate_model_integrity(self, model_path: str) -> bool:
        ...
        has_weight_file = any(os.path.exists(os.path.join(model_path, wf)) for wf in weight_files)
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info(f"内存状态: 使用 {memory_status['used_mb']:.1f}MB / 限制 {memory_status['limit_mb']}MB")
            
            # 检查GPU状态
            if GPUMemoryManager.is_available():
                gpu_info = GPUMemoryManager.get_memory_info()
                if gpu_info:
                    logger.info(f"GPU状态: 已用 {gpu_info['allocated_gb']:.2f}GB / 总计 {gpu_info['total_gb']:.2f}GB")
    ...
    async def embed_texts(self, texts: List[str], model_type: str = None) -> List[List[float]]:
        """
        ...
        with self.monitor_operation(f"embed_texts_{model_type}"):
            try:
                ...
                for i in range(0, len(texts), optimal_batch_size):
                    ...
                    embedding = await self._embedding_service.embed_text(batch, model_type)
                    all_embeddings.extend(embedding)
                
                logger.debug(f"批量文本向量化成功: {len(texts)}个文本, 模型: {model_type}")
    ...
    async def embed_images(self, image_paths: Union[str, List[str]], model_type: str = None) -> List[List[float]]:
        """
        ...
        with self.monitor_operation(f"embed_images_{model_type}"):
            try:
                ...
                embeddings = await self._embedding_service.embed(model_type, image_paths, input_type="image")
                
                logger.debug(f"批量图像向量化成功: {len(image_paths)}个图像, 模型: {model_type}")
                return embeddings
    ...
    async def embed_video_segment(self, video_path: str, start_time: float = 0.0, end_time: Optional[float] = None, aggregation: str = 'mean') -> List[float]:
        """
        ...
        with self.monitor_operation(f"embed_video_segment"):
            try:
                ...
                logger.debug(f"视频片段向量化成功: {video_path} ({start_time}s-{end_time}s)")
                return result
            except FileNotFoundError as e:
    ...
    async def embed_audio(self, audio_path: str, model_type: str = None) -> List[float]:
        ...
        with self.monitor_operation(f"embed_audio_{model_type}"):
            try:
                ...
                embeddings = await self._embedding_service.embed(model_type, [audio_path], input_type="audio")
                result = embeddings[0]
