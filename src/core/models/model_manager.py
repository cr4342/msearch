...
class ModelManager:
    ...
    async def get_model(self, model_type: str) -> 'AsyncEmbeddingEngine':
        ...
        if model_type not in self._models:
            raise ValueError(f"未找到模型: {model_type}。已加载的模型: {list(self._models.keys())}")
        return self._models[model_type]
...
class EmbeddingService:
    ...
    async def embed(
        ...
        # 过滤空输入
        inputs = [input_data for input_data in inputs if input_data and input_data.strip()]
            ...
            if input_type == "text":
                ...
                for audio_path in inputs:
                    ...
                    if os.path.exists(audio_path):
                        ...
                        audio_bytes = audio_preprocessor.get_preprocessed_audio_bytes(audio_path)
                        if audio_bytes:
                ...
                for video_path in inputs:
                    ...
                    if os.path.exists(video_path):
                        ...
                        frames = video_preprocessor.get_video_frames(video_path, max_frames=3)
                        if frames:
                            video_frames.extend(frames)
                            logger.debug(f"视频帧提取成功: {video_path}, {len(frames)}帧")
                        else:
    ...
    async def embed_text(self, texts: Union[str, List[str]], model_type: str) -> List[List[float]]:
        """
        文本向量化
    ...
    async def embed_image(self, images: Union[str, List[str]], model_type: str) -> List[List[float]]:
        """
        图像向量化
    ...
    async def embed_audio(self, audios: Union[str, List[str]], model_type: str) -> List[List[float]]:
        """
        音频向量化
    ...
    async def embed_video(self, videos: Union[str, List[str]], model_type: str) -> List[List[float]]:
        """
        视频向量化
...
if __name__ == "__main__":
    ...
    async def example():
        ...
        embeddings = await service.embed_text(["这是一个测试", "另一个测试"], "image_model")
