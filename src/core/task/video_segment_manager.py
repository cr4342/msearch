...
class VideoSegmentConfig:
    ...
    def __init__(self, config: Dict[str, Any]):
        # 基础切片参数
        self.max_segment_duration = config.get('max_segment_duration', 5.0)  # 最大切片时长（秒）
        self.min_segment_duration = config.get('min_segment_duration', 0.5)  # 最小切片时长（秒）
        self.segment_overlap = config.get('segment_overlap', 0.0)  # 切片重叠时长（秒）
        
        # 场景检测参数
        self.scene_detect_threshold = config.get('scene_detect_threshold', 0.3)  # 场景变化阈值
        self.scene_detect_min_duration = config.get('scene_detect_min_duration', 1.0)  # 最小场景时长
        
        # 时间映射参数
        self.timestamp_precision = config.get('timestamp_precision', 0.1)  # 时间戳精度（秒）
        
        # 短视频处理
        self.short_video_threshold = config.get('short_video_threshold', 6.0)  # 短视频阈值
        self.short_video_handling = config.get('short_video_handling', 'full_process')  # 短视频处理方式
        
        # 分辑率和格式参数
        self.max_resolution_short_edge = config.get('max_resolution_short_edge', 960)  # 短边最大分辑率
...
class VideoSegmentManager:
    ...
    def segment_video(self, video_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Start processing video: {video_path}, duration: {video_duration:.2f}s")
            ...
            if video_duration <= self.config.short_video_threshold:
                ...
                logger.info(f"Video segmentation completed: {video_path}, generated {len(segments)} segments")
                return segments
    ...
    def _generate_timestamp_map(self, start_time: float, end_time: float) -> Dict[str, float]:
        """
    ...
    def _scene_based_segmentation(self, video_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            scene_detect_cmd = [
                'ffmpeg', '-i', video_path,
                '-vf', f'scene_detect=threshold={self.config.scene_detect_threshold}',
                '-f', 'csv', '-'
            ]
            ...
            for i, (start_time, end_time) in enumerate(scenes):
                ...
                if end_time - start_time > self.config.max_segment_duration:
                    ...
                    segment = {
                        ...
                        'is_full_video': False,
                        'timestamp_map': self._generate_timestamp_map(start_time, end_time),
                        'scene_info': {
                            'scene_index': i,
                ...
                try:
                    ...
                    if scene_score > self.config.scene_detect_threshold:
                        if scenes:
                            ...
                            if frame_time - last_scene_end >= self.config.scene_detect_min_duration:
    ...
    def _time_based_segmentation(self, video_path: str, video_duration: float) -> List[Dict[str, Any]]:
        ...
        while current_time < video_duration:
            ...
            end_time = min(current_time + self.config.max_segment_duration, video_duration)
            
            # Ensure slice duration is not below minimum
            if end_time - current_time < self.config.min_segment_duration and end_time < video_duration:
    ...
    def _split_long_segment(self, start_time: float, end_time: float) -> List[Dict[str, Any]]:
        ...
        while current_time < end_time:
            sub_end_time = min(current_time + self.config.max_segment_duration, end_time)
            ...
            segment = {
                ...
                'is_full_video': False,
                'timestamp_map': self._generate_timestamp_map(current_time, sub_end_time),
                'scene_info': {
    ...
    def create_segment_file(self, video_path: str, start_time: float, end_time: float, output_path: str) -> bool:
        ...
        try:
            ...
            cmd = [
                ...
                '-c:v', self.config.video_codec,  # Video encoder
                '-c:a', self.config.audio_codec,  # Audio encoder
    ...
    def validate_config(self) -> bool:
        ...
        if self.config.min_segment_duration > self.config.max_segment_duration:
            logger.error("Minimum slice duration cannot be greater than maximum slice duration")
            return False
        ...
        if self.config.scene_detect_threshold < 0 or self.config.scene_detect_threshold > 1:
