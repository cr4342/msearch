...
class OptimizedConcurrencyManager:
    ...
    def __init__(self, config: ConcurrencyConfig, device: str = 'cpu'):
        ...
        logger.info(f"优化并发管理器初始化完成: mode={config.concurrency_mode}, "
    ...
    def _get_gpu_info(self) -> tuple[float, float]:
        ...
        try:
            ...
            memory_percent = (allocated_memory / total_memory) * 100 if total_memory > 0 else 0.0
    ...
    def _adjust_concurrent_count(self, resources: SystemResources) -> None:
        ...
        with self.lock:
            ...
            memory_too_high = resources.memory_percent > self.config.target_memory_percent
            gpu_too_high = (self.has_gpu and 
                          resources.gpu_memory_percent > self.config.target_gpu_memory_percent)
            ...
            if cpu_too_high or memory_too_high or gpu_too_high:
                if current_concurrent > self.config.min_concurrent:
                    ...
                    logger.debug(f"资源使用过高，减少并发数: {current_concurrent} -> {self.current_concurrent}")
            
            # 如果资源使用适中且有容量，增加并发数
            else:
                cpu_ok = resources.cpu_percent < self.config.target_cpu_percent * 0.8
                memory_ok = resources.memory_percent < self.config.target_memory_percent * 0.8
                gpu_ok = (not self.has_gpu or 
                         resources.gpu_memory_percent < self.config.target_gpu_memory_percent * 0.8)
                ...
                if cpu_ok and memory_ok and gpu_ok:
                    if current_concurrent < self.config.max_concurrent:
                        ...
                        logger.debug(f"资源使用正常，增加并发数: {current_concurrent} -> {self.current_concurrent}")
    ...
    def set_concurrent_count(self, count: int) -> bool:
        ...
        if count < self.config.min_concurrent or count > self.config.max_concurrent:
            logger.warning(f"并发数 {count} 超出配置范围 [{self.config.min_concurrent}, {self.config.max_concurrent}]")
            return False
