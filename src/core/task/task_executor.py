...
class OptimizedTaskExecutor:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if success:
                    ...
                    # 重试前等待
                    logger.info(
                        f"任务执行失败，准备重试 ({attempt + 1}/{self.max_retries}): {task.id}"
                    )
                ...
                if attempt < self.max_retries:
                    logger.info(
                        f"准备重试 ({attempt + 1}/{self.max_retries}): {task.id}"
                    )
                    time.sleep(self.retry_delay)
                    continue
                else:
    ...
    def execute_batch_tasks(
        self, tasks: list[Task], max_concurrent: int = 4
    ) -> Dict[str, Any]:
        """
        批量执行任务
        ...
        Returns:
            执行结果统计
        """
        results = {"completed": 0, "failed": 0, "total": len(tasks)}
