...
class OptimizedGroupManager:
    ...
    def update_task_status_in_group(self, task_id: str, old_status: str, new_status: str) -> None:
        """
        更新组中任务的状态
    ...
    def is_group_complete(self, file_id: str) -> bool:
        ...
        with self.lock:
            ...
            return stats.get('completed', 0) == stats.get('total', 0) and stats.get('total', 0) > 0
    ...
    def get_running_groups(self) -> List[str]:
        ...
        """
        with self.lock:
            return [gid for gid, status in self.group_status.items() if status == 'running']
    ...
    def get_groups_with_status(self, status: str) -> List[str]:
        ...
        """
        with self.lock:
            return [gid for gid, group_status in self.group_status.items() 
