...
class PipelineLockManager:
            ...
            if task.file_id in self.pipeline_locks:
                ...
                if self._is_lock_expired(current_lock):
                    ...
                    if task.id not in self.waiting_queues[task.file_id]:
                        ...
                        logger.debug(f"任务 {task.id} 加入文件 {task.file_id} 的流水线等待队列")
    ...
    def release_pipeline_lock(self, task: Task) -> None:
        ...
        with self.lock:
            ...
            if current_lock['holder'] == task.id:
                ...
                has_waiting_pipeline_tasks = self._has_pipeline_tasks_in_queue(task.file_id)
    ...
    def cleanup_expired_locks(self) -> int:
        ...
        with self.lock:
            ...
            if expired_locks:
                logger.warning(f"清理了 {len(expired_locks)} 个过期的流水线锁: {expired_locks}")
    ...
    def get_statistics(self) -> Dict[str, any]:
        ...
        with self.lock:
            return {
                ...
                'waiting_tasks_total': sum(len(q) for q in self.waiting_queues.values())
            }