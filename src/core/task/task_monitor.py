...
class OptimizedTaskMonitor:
    ...
    def get_all_tasks(self, status: str = None, task_type: str = None) -> Dict[str, Task]:
        """
        获取所有任务
        ...
        with self.lock:
            ...
            for task_id, task in list(self.tasks.items()):
                ...
                if task.status in ['completed', 'failed', 'cancelled']:
                    task_time = getattr(task, 'completed_at', getattr(task, 'updated_at', None))
                    if task_time:
                        task_timestamp = task_time.timestamp() if hasattr(task_time, 'timestamp') else task_time
    ...
    def search_tasks(self, query: str) -> List[Task]:
        ...
        with self.lock:
            ...
            for task in self.tasks.values():
                # 搜索任务ID、类型、错误信息等
                if (query.lower() in task.id.lower() or
                    query.lower() in task.task_type.lower() or
                    (task.error and query.lower() in task.error.lower())):
                    results.append(task)
