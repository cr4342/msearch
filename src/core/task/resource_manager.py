...
class OptimizedResourceManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.memory_warning_threshold = config.get(
            "memory_warning_threshold", 85.0
        )  # 警告级（85%）
        self.memory_pause_threshold = config.get(
            "memory_pause_threshold", 95.0
        )  # 暂停级（95%）

        # GPU内存阈值（根据设计文档）
        self.gpu_memory_warning_threshold = config.get(
    ...
    def check_resource_usage(self) -> str:
        ...
        with self.lock:
            ...
            if memory_usage >= self.memory_pause_threshold or (
                ...
                gpu_memory_usage
                and gpu_memory_usage >= self.gpu_memory_warning_threshold
            ):
            ...
            if current_state != self.current_state:
                ...
                if current_state == "warning":
                    logger.warning(
                        f"资源警告: 内存使用率 {memory_usage:.1f}% "
                        f"(阈值 {self.memory_warning_threshold}%)"
                    )
                elif current_state == "pause":
                    logger.error(
                        f"资源暂停: 内存使用率 {memory_usage:.1f}% "
    ...
    def _record_resource_usage(
        self, memory_usage: float, gpu_memory_usage: Optional[float], state: str
    ) -> None:
        """记录资源使用情况到历史"""
        record = {
            "timestamp": time.time(),
    ...
    def get_recommendations(self) -> Dict[str, Any]:
        ...
        with self.lock:
            current_state = self.current_state
            recommendations = {"state": current_state, "actions": []}
