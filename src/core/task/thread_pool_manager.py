...
class ThreadPoolManager:
    ...
    def _init_pools(self):
        ...
        if "embedding" in self.config:
            ...
            logger.info(
                f"Embedding Pool 初始化: max_workers={embedding_config.get('max_workers', 4)}, "
        ...
        if "io" in self.config:
            ...
            logger.info(
                f"I/O Pool 初始化: max_workers={io_config.get('max_workers', 8)}, "
        ...
        if "task" in self.config:
            ...
            logger.info(
                f"Task Pool 初始化: max_workers={task_config.get('max_workers', 8)}, "
        ...
        with self._lock:
            for name, pool in self.pools.items():
                stats[name] = {
                    ...
                    "pending_tasks": (
                        pool._work_queue.qsize() if hasattr(pool, "_work_queue") else 0
                    ),
                }
        return stats
