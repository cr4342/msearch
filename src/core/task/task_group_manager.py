...
class TaskGroupManager:
        ...
        async with self._lock:
            if file_id not in self.task_groups:
                ...
                self.task_groups[file_id] = TaskGroup(
                    file_id=file_id, file_path=task.file_path
                )
    ...
    def acquire_pipeline_lock(self, file_id: str, task_id: str) -> bool:
        ...
        if file_id not in self.pipeline_locks:
            ...
            self.pipeline_locks[file_id] = PipelineLock(
                owner_file=file_id, owner_task=task_id, acquired_at=time.time()
            )
    ...
    def get_lock_status(self, file_id: str) -> Optional[Dict[str, Any]]:
        ...
        lock = self.pipeline_locks.get(file_id)
        if lock:
            return {"locked": True, "lock_info": lock.to_dict()}
        return {"locked": False}
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            pending_tasks = sum(
                len(g.get_pending_tasks()) for g in self.task_groups.values()
            )
            running_tasks = sum(
                len(g.get_running_tasks()) for g in self.task_groups.values()
            )
            completed_tasks = sum(
                len(g.get_completed_tasks()) for g in self.task_groups.values()
            )
            failed_tasks = sum(
                len(g.get_failed_tasks()) for g in self.task_groups.values()
            )
            ...
            return {
                ...
                "average_progress": (
                    sum(g.get_progress() for g in self.task_groups.values())
                    / total_groups
                    if total_groups > 0
                    else 0.0
                ),
