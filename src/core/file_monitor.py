...
class FileMonitor:
        ...
        while self.is_running:
            try:
                ...
                for directory in self.monitored_directories:
                    self._scan_directory(directory, file_states)
    ...
    def _scan_directory(self, directory: str, file_states: Dict[str, float]) -> None:
        ...
        try:
            ...
            for root, dirs, files in os.walk(directory):
                ...
                # 检查已删除的文件
                self._check_deleted_files(directory, current_files, file_states)
        except Exception as e:
    ...
    def _on_file_created(self, file_path: str) -> None:
        ...
        # 调用所有注册的创建事件处理器
        for handler in self.event_handlers['created']:
            try:
    ...
    def _on_file_modified(self, file_path: str) -> None:
        ...
        # 调用所有注册的修改事件处理器
        for handler in self.event_handlers['modified']:
            try:
    ...
    def _on_file_deleted(self, file_path: str) -> None:
        ...
        # 调用所有注册的删除事件处理器
        for handler in self.event_handlers['deleted']:
            try:
