...
class MSearchCLI:
    ...
    def search_text(self, query: str, top_k: int = 20):
        ...
        result = self._request("POST", "/api/v1/search/text", 
    ...
    def search_image(self, image_path: str, top_k: int = 20):
        ...
        with open(image_path, 'rb') as f:
            ...
            result = self._request("POST", "/api/v1/search/image", 
                                  files=files, data={"top_k": top_k})
    ...
    def search_audio(self, audio_path: str, top_k: int = 20):
        ...
        with open(audio_path, 'rb') as f:
            ...
            result = self._request("POST", "/api/v1/search/audio", 
                                  files=files, data={"top_k": top_k})
    ...
    def update_priority(self, task_id: str, priority: int):
        ...
        result = self._request("POST", f"/api/v1/tasks/{task_id}/priority",
    ...
    def cancel_all_tasks(self, cancel_running: bool = False):
        ...
        result = self._request("POST", "/api/v1/tasks/cancel-all",
    ...
    def cancel_tasks_by_type(self, task_type: str, cancel_running: bool = False):
        ...
        result = self._request("POST", "/api/v1/tasks/cancel-by-type",
    ...
    def index_file(self, file_path: str):
        ...
        # 创建索引任务
        result = self._request("POST", "/api/v1/index/file",
        ...
        if result.get('task_id'):
            print(f"\n您可以使用以下命令查看任务进度:")
            print(f"  python src/cli.py task get {result.get('data', {}).get('task_id')}")
    ...
    def index_directory(self, directory: str, recursive: bool = True):
        ...
        # 创建索引任务
        result = self._request("POST", "/api/v1/index/directory",
...
示例:
  ...
  python src/cli.py index status
    ...
    cancel_all_parser.add_argument("--cancel-running", action="store_true", 
                                  help="同时取消正在运行的任务")
    
    # 按类型取消任务
    cancel_type_parser = task_subparsers.add_parser("cancel-by-type", help="按类型取消任务")
    cancel_type_parser.add_argument("task_type", help="任务类型")
    cancel_type_parser.add_argument("--cancel-running", action="store_true",
                                   help="同时取消正在运行的任务")
    ...
    index_dir_parser.add_argument("--no-recursive", action="store_true",
                                  help="不递归索引子目录")
    ...
    try:
        if args.command == "health":
            ...
            if args.task_action == "stats":
                ...
                cli.list_tasks(status=args.status, task_type=args.type, limit=args.limit)
                ...
                cli.cancel_tasks_by_type(args.task_type, cancel_running=args.cancel_running)
            else:
                parser.print_help()
                sys.exit(1)
