...
class MSearchWebUI:
    ...
    def _on_file_created(self, event_type: str, file_path: str):
        ...
        try:
            ...
            logger.info(
                f"[文件监控] 已提交处理任务: {file_path} -> {response.get('message', 'Success')}"
            )
    ...
    def _on_file_modified(self, event_type: str, file_path: str):
        ...
        try:
            ...
            response = self.api_client.index_file(file_path)
            logger.info(
                f"[文件监控] 已重新提交处理任务: {file_path} -> {response.get('message', 'Success')}"
            )
        ...
        async def _search():
            try:
                ...
                response = self.api_client.search_text(
                    query=query, top_k=top_k, threshold=similarity_threshold
                )
                ...
                for i, result in enumerate(sorted_results):
                    file_name = result.get(
                        ...
                        result.get("metadata", {}).get(
                            "file_name", result.get("file_path", "未知")
                        ),
                    )
                    file_path = result.get(
                        "file_path", result.get("metadata", {}).get("file_path", "未知")
                    )
                    ...
                    # 格式化相似度为百分比
                    similarity_percent = (
                        similarity * 100 if similarity <= 1 else similarity
                    )
                    similarity_bar = "█" * int(similarity_percent / 10) + "░" * (
                        10 - int(similarity_percent / 10)
                    )
    ...
    def _add_to_history(self, query: str, search_type: str):
        ...
        history_item = {"query": query, "type": search_type, "timestamp": time.time()}
    ...
    def get_search_history(self) -> str:
        ...
        for i, item in enumerate(self.search_history[:20]):
            ...
            # 根据类型添加图标
            type_icon = {"text": "🔍", "image": "🖼️"}.get(search_type, "📄")
    ...
    async def search_image(
        self, image_path: str, top_k: int = 10, similarity_threshold: float = 0.0
    ):
        """
        图像搜索
        ...
        try:
            ...
            # 调用API进行搜索
            response = self.api_client.search_image(image_path=image_path, top_k=top_k)
    ...
    def get_system_status(self) -> str:
        ...
        model_name = self.config_manager.get(
            "models.available_models.chinese_clip_large.model_name", "未知"
        )
        model_path = self.config_manager.get(
            "models.available_models.chinese_clip_large.local_path", "未知"
        )
        embedding_dim = self.config_manager.get(
            "models.available_models.chinese_clip_large.embedding_dim", "未知"
        )
        device = self.config_manager.get(
            "models.available_models.chinese_clip_large.device", "未知"
        )
    ...
    def _normalize_tasks(self, tasks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for task in tasks:
            ...
            created_at = self._to_timestamp(task.get("created_at"))
            started_at = self._to_timestamp(task.get("started_at"))
        ...
        with gr.Blocks(title="msearch 多模态检索系统") as demo:
            ...
            with gr.Tab("🔍 文本搜索"):
                with gr.Row():
                    ...
                    with gr.Column():
                        ...
                        show_thumbnails = gr.Checkbox(label="显示缩略图", value=True)
            ...
            with gr.Tab("🖼️ 图像搜索"):
                with gr.Row():
                    image_input = gr.Image(
                        label="上传图像", type="filepath", height=400
                    )
                    ...
                    with gr.Column():
                        ...
                        search_btn_img = gr.Button(
                            "🔍 搜索相似图像", variant="primary", size="lg"
                        )
                ...
                search_btn_img.click(
                    ...
                    inputs=[
                        image_input,
                        top_k_slider_img,
                        similarity_threshold_slider_img,
                    ],
            ...
            with gr.Tab("📊 系统状态"):
                ...
                status_output = gr.Textbox(
                    label="系统状态信息", lines=30, interactive=False
                )

                status_btn.click(fn=self.get_system_status, outputs=status_output)

                # 自动加载状态
                demo.load(fn=self.get_system_status, outputs=status_output)
            ...
            with gr.Tab("📋 任务管理器"):
                ...
                with gr.Row():
                    task_search = gr.Textbox(
                        label="搜索任务", placeholder="输入任务ID或文件路径...", scale=3
                    )
                ...
                with gr.Row():
                    with gr.Column(scale=1):
                        total_tasks = gr.Number(
                            label="总任务数", value=0, interactive=False
                        )
                    with gr.Column(scale=1):
                        pending_tasks = gr.Number(
                            label="待处理", value=0, interactive=False
                        )
                    with gr.Column(scale=1):
                        running_tasks = gr.Number(
                            label="运行中", value=0, interactive=False
                        )
                    with gr.Column(scale=1):
                        completed_tasks = gr.Number(
                            label="已完成", value=0, interactive=False
                        )
                    with gr.Column(scale=1):
                        failed_tasks = gr.Number(
                            label="失败", value=0, interactive=False
                        )
                    with gr.Column(scale=1):
                        paused_tasks = gr.Number(
                            label="已暂停", value=0, interactive=False
                        )
                ...
                with gr.Row():
                    with gr.Column(scale=1):
                        success_rate = gr.Textbox(
                            label="成功率", value="0%", interactive=False
                        )
                    with gr.Column(scale=1):
                        avg_duration = gr.Textbox(
                            label="平均耗时", value="0s", interactive=False
                        )
                    with gr.Column(scale=1):
                        throughput = gr.Textbox(
                            label="吞吐量", value="0/min", interactive=False
                        )
                    with gr.Column(scale=1):
                        queue_depth = gr.Number(
                            label="队列深度", value=0, interactive=False
                        )
                    with gr.Column(scale=1):
                        system_load = gr.Textbox(
                ...
                with gr.Accordion("任务详情", open=False):
                    with gr.Tabs():
                        ...
                        with gr.Tab("日志输出"):
                            task_logs = gr.Textbox(
                                label="任务日志", lines=10, interactive=False
                            )
                        with gr.Tab("依赖关系"):
                ...
                operation_result = gr.Textbox(
                    label="操作结果", lines=2, interactive=False
                )
                ...
                select_all_btn.click(
                    fn=self.select_all_tasks, inputs=task_list, outputs=task_list
                )

                deselect_all_btn.click(
                    fn=self.deselect_all_tasks, inputs=task_list, outputs=task_list
                )
                ...
                set_priority_btn.click(
                    ...
                    inputs=[
                        task_list,
                        gr.Number(label="新优先级", minimum=1, maximum=10),
                    ],
                    outputs=[task_list, operation_result],
                )
                ...
                export_btn.click(
                    ...
                    inputs=[
                        task_list,
                        gr.Dropdown(
                            label="导出格式", choices=["CSV", "JSON"], value="CSV"
                        ),
                    ],
            ...
            with gr.Tab("📜 搜索历史"):
                ...
                history_output = gr.Markdown(
                    label="搜索历史", value="## 📜 搜索历史\n\n暂无搜索记录"
                )

                history_btn.click(fn=self.get_search_history, outputs=history_output)

                clear_history_btn.click(
                    fn=self.clear_search_history, outputs=history_output
                )

                # 自动加载历史记录
                demo.load(fn=self.get_search_history, outputs=history_output)
                ...
                with gr.Row():
                    with gr.Column():
                        ...
                        full_scan_output = gr.Textbox(
                            label="扫描结果", lines=10, interactive=False
                        )
                    ...
                    with gr.Column():
                        ...
                        max_concurrent_slider = gr.Slider(
                            minimum=1, maximum=16, value=4, step=1, label="最大并发数"
                        )
                        start_vectorization_btn = gr.Button(
                            "🚀 启动向量化处理", variant="primary"
                        )
                        vectorization_output = gr.Textbox(
                            label="处理结果", lines=10, interactive=False
                        )
                ...
                with gr.Row():
                    ...
                    with gr.Column():
                        ...
                        priority_task_id_input = gr.Textbox(
                            label="任务ID", placeholder="输入要调整优先级的任务ID"
                        )
                        ...
                        update_priority_btn = gr.Button(
                            "⬆️ 更新优先级", variant="secondary"
                        )
                        update_priority_output = gr.Textbox(
                            label="更新结果", lines=3, interactive=False
                        )
