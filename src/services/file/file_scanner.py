...
class FileScanner:
    ...
    def get_deleted_files(
        self, directories: List[str], known_files: Set[str]
    ) -> List[str]:
        """
        获取已删除文件
    ...
    def get_modified_files(
        self, directories: List[str], known_files: Dict[str, float]
    ) -> List[str]:
        """
        获取已修改的文件
    ...
    def filter_by_size(
        self, file_paths: List[str], min_size: int = 0, max_size: int = None
    ) -> List[str]:
        """
        按文件大小过滤
    ...
    def filter_by_extension(
        self, file_paths: List[str], extensions: Set[str]
    ) -> List[str]:
        """
        按文件扩展名过滤
        ...
        filtered_files = [
            f for f in file_paths if Path(f).suffix.lower() in extensions_lower
        ]
