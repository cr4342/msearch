...
class FileIndexer:
    ...
    def index_file(
        ...
        try:
            ...
            if duplicate_file_id:
                if self.logger:
                    self.logger.info(
                        f"文件重复，已跳过: {file_path}, 重复文件ID: {duplicate_file_id}"
                    )
    ...
    def index_files(
        self, file_paths: List[str], submit_task: bool = True
    ) -> List[FileMetadata]:
        """
        批量索引文件
            ...
            if file_path in self.file_index:
                ...
                if extracted_metadata:
                    ...
                    metadata = FileMetadata(
                        id=file_id,
                        file_path=extracted_metadata.get("file_path", file_path),
                        file_name=extracted_metadata.get(
    ...
    def _submit_processing_task(self, metadata: FileMetadata) -> Optional[str]:
        ...
        try:
            ...
            task_type = task_type_map.get(
                metadata.file_type.value, "file_embed_unknown"
            )
            ...
            task_id = self.task_manager.create_task(
                task_type=task_type, task_data=task_data, priority=5, max_retries=3
            )

            if self.logger:
                self.logger.info(
                    f"任务提交成功: {task_id}, 文件: {metadata.file_path}, 任务类型: {task_type}"
                )
    ...
    def index_directory(
        ...
        try:
            ...
            if self.logger:
                self.logger.info(
                    f"目录索引完成: {directory}, 共索引 {len(metadata_list)} 个文件"
                )
    ...
    def get_index_stats(self) -> Dict[str, Any]:
        ...
        Returns:
            统计信息字典
        """
        stats = {"total_files": len(self.indexed_files), "by_status": {}, "by_type": {}}
