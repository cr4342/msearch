...
class VideoSceneDetector:
    ...
    def detect_scenes(self, video_path: str) -> List[Tuple[int, float, float]]:
        ...
        cmd = [
            ...
            video_path,
            "-vf",
            f"select='gt(scene,{self.scene_threshold})',metadata=print:file=pipe:1",
            "-f",
            "null",
            "-",
        ]
    ...
    def get_scene_segments(
        self, video_path: str, max_duration: float = 5.0
    ...
    def set_scene_threshold(self, threshold: float) -> None:
        ...
        if 0 <= threshold <= 1:
            ...
            logger.warning(
                f"Invalid scene threshold: {threshold}. Must be between 0 and 1."
            )
