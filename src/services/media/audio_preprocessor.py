...
class AudioPreprocessor:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        # 音频预处理参数（按照设计文档要求）
        self.target_sample_rate = 48000  # CLAP要求48kHz采样率
        self.target_channels = 1  # 单声道
        self.target_format = 'wav'  # WAV格式
        self.min_duration = 3.0  # 最小音频时长（秒）
    ...
    def process_audio(self, audio_path: str, output_path: Optional[str] = None) -> Dict[str, Any]:
        """
        处理音频文件
        ...
        try:
            ...
            try:
                ...
                if duration < self.min_duration:
                    logger.info(f"Audio too short ({duration}s < {self.min_duration}s), skipping")
                    return {
                        'status': 'skipped',
    ...
    def _preprocess_audio(self, audio_path: str, output_path: Optional[str] = None) -> Optional[str]:
        """
        音频预处理
        ...
        try:
            ...
            if not output_path:
                ...
                output_path = os.path.join(temp_dir, f"{file_name}_processed.wav")
            ...
            if sr != self.target_sample_rate:
                audio_data = librosa.resample(audio_data, orig_sr=sr, target_sr=self.target_sample_rate)
            
            # 保存为WAV格式
            sf.write(output_path, audio_data, self.target_sample_rate)
            
            logger.info(f"Audio preprocessed: {audio_path} -> {output_path}")
            logger.info(f"  Original: {sr}Hz, {audio_data.shape if len(audio_data.shape) > 1 else 'mono'}")
