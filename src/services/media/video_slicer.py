...
class VideoSlicer:
    ...
    def __init__(self, strategy: str = "hybrid", **kwargs):
        ...
        config_dict = {
            "max_segment_duration": kwargs.get(
                "max_segment_duration", 5.0
            ),  # 最大切片时长（秒）
            "min_segment_duration": kwargs.get(
                "min_segment_duration", 0.5
            ),  # 最小切片时长（秒）
            "segment_overlap": kwargs.get("segment_overlap", 0.0),  # 切片重叠时间（秒）
            "scene_detect_threshold": kwargs.get(
                "scene_detect_threshold", 0.3
            ),  # 场景变化阈值
            "scene_detect_min_duration": kwargs.get(
                "scene_detect_min_duration", 1.0
            ),  # 最小场景时长
            "timestamp_precision": kwargs.get(
                "timestamp_precision", 0.1
            ),  # 时间戳精度（秒）
            "short_video_threshold": kwargs.get(
                "short_video_threshold", 6.0
            ),  # 短视频阈值
            "short_video_handling": kwargs.get(
                "short_video_handling", "full_process"
            ),  # 短视频处理方式
            "max_resolution_short_edge": kwargs.get(
                "max_resolution_short_edge", 960
            ),  # 短边最大分辨率
            "scene_detect_enabled": kwargs.get(
    ...
    def create_segment_file(
        ...
        Returns:
            是否成功
        """
        return self.video_segment_manager.create_segment_file(
