...
def check_duplicate_file(
    file_path: str, existing_hashes: Dict[str, Dict[str, Any]]
) -> Optional[str]:
    """
    检查文件是否重复
...
class MediaInfoHelper:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        audio_min_duration = processing_audio.get("min_duration", 3.0)

        # 媒体价值阈值配置
        self.min_media_value_duration = {
            "audio": float(audio_min_duration),  # 音频超过配置阈值才有价值
    ...
    def _get_duration_with_ffprobe(self, file_path: str) -> Optional[float]:
        """使用ffprobe获取媒体时长"""
        try:
            ...
            logger.debug(f"ffprobe获取时长失败: {file_path}, {e}")
            return None
        ...
        try:
            ...
            if media_type == "video":
                try:
                    ...
                    logger.debug(f"cv2获取视频时长失败: {file_path}, {e}")
                ...
                try:
                    ...
                    logger.debug(f"librosa获取音频时长失败: {file_path}, {e}")
