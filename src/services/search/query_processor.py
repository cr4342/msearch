...
class QueryProcessor:
    ...
    def _validate_query(self, query: SearchQuery) -> bool:
        ...
        if (
            ...
            and not query.query_image
            and not query.query_audio
            and not query.query_vector
        ):
            return False
        ...
        if query.query_text:
            if (
                len(query.query_text) < self.min_query_length
                or len(query.query_text) > self.max_query_length
            ):
                return False
