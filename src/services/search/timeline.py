...
class VideoTimelineResult:
    ...
    timeline_items: List[VideoTimelineItem] = field(default_factory=list)  # 时间轴条目列表
    grouped_by_video: Optional[Dict[str, List[VideoTimelineItem]]] = None  # 按视频分组的条目
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "sorted_by_relevance": [item.to_dict() for item in self.sorted_by_relevance],
            "metadata": self.metadata
        }
...
class VideoTimelineGenerator:
    ...
    def from_search_results(
        ...
        logger.info(f"生成时间轴结果: {result.total_results} 个片段, {result.get_video_count()} 个视频")
    ...
    def from_database_results(
        ...
        logger.info(f"从数据库生成时间轴结果: {result.total_results} 个片段, {result.get_video_count()} 个视频")
        
        return result
    
    def merge_timeline_results(
        self,
        results: List[VideoTimelineResult],
        ...
        if merge_strategy == "union":
            ...
            for result in results:
                for item in result.timeline_items:
                    # 检查是否已存在相同的条目
                    exists = any(
                        existing.video_uuid == item.video_uuid and
                        existing.start_time == item.start_time and
            ...
            if len(results) < 2:
                ...
                for item in first_result.timeline_items:
                    ...
                    exists_in_all = all(
                        any(
                            other_item.video_uuid == item.video_uuid and
                            other_item.start_time == item.start_time and
                            other_item.end_time == item.end_time
        ...
        logger.info(f"合并时间轴结果: {len(results)} 个结果 -> {merged.total_results} 个片段")
