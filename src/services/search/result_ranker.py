...
class ResultRanker:
    ...
    def rank_results(self, results: List[SearchResultItem], reverse: bool = True) -> List[SearchResultItem]:
        """
        对结果进行排序
        ...
        try:
            # 按相似度排序
            sorted_results = sorted(results, key=lambda x: x.similarity, reverse=reverse)
    ...
    def fuse_results(self, multimodal_result: MultimodalSearchResult) -> List[SearchResultItem]:
        """
        融合多模态搜索结果
        ...
        try:
            ...
            if multimodal_result.text_results:
                all_results.extend([(r, "text", multimodal_result.weights.get("text", 1.0)) for r in multimodal_result.text_results])
            
            if multimodal_result.image_results:
                all_results.extend([(r, "image", multimodal_result.weights.get("image", 1.0)) for r in multimodal_result.image_results])
            
            if multimodal_result.audio_results:
                all_results.extend([(r, "audio", multimodal_result.weights.get("audio", 1.0)) for r in multimodal_result.audio_results])
            
            if multimodal_result.video_results:
                all_results.extend([(r, "video", multimodal_result.weights.get("video", 1.0)) for r in multimodal_result.video_results])
            
            if multimodal_result.face_results:
                all_results.extend([(r, "face", multimodal_result.weights.get("face", 1.0)) for r in multimodal_result.face_results])
    ...
    def _fuse_by_file_id(self, weighted_results: List[tuple]) -> List[SearchResultItem]:
        ...
        for result, modality, weight in weighted_results:
            ...
            file_groups[file_id]["results"].append((result, modality, weighted_similarity))
            file_groups[file_id]["max_similarity"] = max(file_groups[file_id]["max_similarity"], weighted_similarity)
    ...
    def deduplicate_results(self, results: List[SearchResultItem]) -> List[SearchResultItem]:
        """
        去重搜索结果
    ...
    def filter_by_threshold(self, results: List[SearchResultItem], threshold: float) -> List[SearchResultItem]:
        """
        按阈值过滤结果
    ...
    def apply_weights(self, results: List[SearchResultItem], weights: Dict[str, float]) -> List[SearchResultItem]:
        """
        应用权重到结果
    ...
    def normalize_scores(self, results: List[SearchResultItem]) -> List[SearchResultItem]:
        """
        归一化分数
        ...
        try:
            ...
            if max_score - min_score > 0:
                for result in results:
                    result.similarity = (result.similarity - min_score) / (max_score - min_score)
    ...
    def re_rank_by_recency(self, results: List[SearchResultItem], recency_weight: float = 0.1) -> List[SearchResultItem]:
        """
        按时间重新排序
        ...
        try:
            ...
            if timestamps and max(timestamps) > 0:
                for result in results:
                    ...
                    # 融合相似度分数和时间分数
                    result.similarity = (1 - recency_weight) * result.similarity + recency_weight * time_score
    ...
    def get_top_k(self, results: List[SearchResultItem], k: int) -> List[SearchResultItem]:
        """
        获取前K个结果
