...
**用户故事:** 作为用户，我希望系统能够自动监控目录并处理新文件，以便无需手动触发索引更新。

#### 验收标准

1. WHEN 用户配置监控目录时，THE MSearch系统 SHALL 为该目录启动Watchdog监控器
2. WHEN Watchdog监控器检测到新文件时，THE MSearch系统 SHALL 在30秒内开始处理
3. WHEN Watchdog监控器检测到文件删除或移动时，THE MSearch系统 SHALL 更新SQLite数据库并从Qdrant数据库中移除对应的向量
4. WHEN MSearch系统重启时，THE MSearch系统 SHALL 自动恢复所有已配置目录的监控状态
5. IF MSearch系统遇到处理错误，THEN MSearch系统 SHALL 记录错误日志并继续处理其他文件
