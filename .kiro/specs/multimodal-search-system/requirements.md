...
**用户故事:** 作为视频剪辑师，我希望系统能够自动将视频文件优化为对AI模型友好的格式，并通过降采样、场景分割和聚合策略降低内存消耗，以便检索高效且避免大模型处理时出现内存溢出，同时保持原始素材不变。

**核心目标:**
- **格式优化**: 将原始视频转换为CLIP/Whisper等模型的标准输入格式（H.264 MP4、8fps、RGB色彩空间）
- **内存保护**: 通过分辨率限制、场景分割、MaxSim聚合等策略，防止大模型处理时爆内存
