...
def get_thumbnail_filename(self, file_hash: str) -> str:
    ...
    Returns:
        缩略图文件名（格式：{hash_prefix}.jpg）
    """
    # 使用文件哈希的前16位作为文件名
    hash_prefix = file_hash[:16]
    return f"{hash_prefix}.jpg"
...
def get_thumbnail_path_by_hash(self, file_hash: str) -> str:
    ...
    Returns:
        缩略图文件完整路径
    """
    filename = self.get_thumbnail_filename(file_hash)
...
def has_thumbnail_by_hash(self, file_hash: str) -> bool:
    ...
    Returns:
        是否存在缩略图
    """
    thumbnail_path = self.get_thumbnail_path_by_hash(file_hash)
    return os.path.exists(thumbnail_path)
```

**缩略图命名优势**：
- **避免重复生成**：相同内容的文件共享同一个缩略图文件
- **路径无关性**：文件移动或重命名不影响缩略图文件
- **缓存高效**：基于哈希的缓存管理更高效
- **存储优化**：减少重复缩略图文件的存储空间
- **去重支持**：与文件哈希去重机制完美配合
